...
object Req4BlackListApplication {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 准备配置对象
    val sparkConf: SparkConf = new SparkConf().setAppName("Req4BlackListApplication").setMaster("local[*]")
    ...
    val messageDStream: DStream[MyKafkaMessage] = kafkaDStream.map((record: ConsumerRecord[String, String]) => {
    ...
    */
    ...
    // Coding : Driver 1
    val transformDStream: DStream[MyKafkaMessage] = messageDStream.transform((rdd: RDD[MyKafkaMessage]) => {
      // Coding : Driver n
      val client: Jedis = RedisUtil.getJedisClient
      val blackList: util.Set[String] = client.smembers("blacklist")
      ...
      rdd.filter((message: MyKafkaMessage) => {
        // Coding : Executor m
        // Driver ==> Executor
    ...
    val dateAdvUserToCountDStream: DStream[(String, Long)] = transformDStream.map((message: MyKafkaMessage) => {
    ...
    stateDStream.foreachRDD((rdd: RDD[(String, Long)]) => {
      rdd.foreach {
        case (key, sum) => {
          if (sum >= 100) {
