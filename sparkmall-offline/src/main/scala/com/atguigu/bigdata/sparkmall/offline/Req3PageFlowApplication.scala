...
object Req3PageFlowApplication {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val pageToCountRDD: RDD[(Long, Long)] = pageRDD.map(action=>{( action.page_id, 1L )})
        ...
        val sessionGroupRDD: RDD[(String, Iterable[UserVisitAction])] = userVisitActionRDD.groupBy(action=>{action.session_id})
